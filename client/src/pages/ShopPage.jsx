...
const BgPreview = ({ bgId }) => {
  const BgC = BACKGROUND_COMPONENTS[bgId];
  return (
    <div style={{ width: '100%', height: '100%', borderRadius: 10, position: 'relative', overflow: 'hidden', transform: 'translateZ(0)' }}>
      {BgC && <BgC />}
    </div>
  );
};
      ...
      <div style={{ display: 'flex', gap: 8, padding: '14px 16px 4px', overflowX: 'auto', scrollbarWidth: 'none' }}>
        {CATS.map(c => (
          <motion.button key={c.id} onClick={() => setCat(c.id)} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
            style={{
              display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 99, border: 'none', flexShrink: 0,
